...
export async function createMatch(req, res) {
	try {
		...
		if (
			...
			console.log(
				"Controller ERROR: Difficulty should be either easy, medium, or hard."
			);
		...
		if (difficulty && currentUser && currentUserSocketId) {
			...
			if (pendingMatchExist) {
				...
				const pendingMatchUserSocketId =
					pendingMatchExist.user1SocketId;
				const pendingMatchCollabRoomSocketId =
					pendingMatchExist.collabRoomSocketId;
				...
				if (matchDeleted.err) {
					...
					return res.status(400).json({
						message: "Controller ERROR: Could not delete match!",
					});
				} else {
					console.log(
						`Deleted pendingMatch for ${pendingMatchUser} successfully!`
					);
				}
				...
				const matchCreated = await _createMatch(
					isPendingMatch,
					difficulty,
					currentUser,
					pendingMatchUser,
					currentUserSocketId,
					pendingMatchUserSocketId,
					pendingMatchCollabRoomSocketId
				);
				if (matchCreated.err) {
					console.log(
				...
				console.log(
					"pendingMatch does not exist. Making a new pendingMatch."
				);
	...
	try {
		...
		if (currentUser) {
			...
			if (!matchExistUser1 && !matchExistUser2) {
				...
				return res.status(409).json({
					message: `Controller ERROR: currentUser (${currentUser}) is not in any match and cannot be deleted.`,
				});
			}
			...
			if (matchDeleted.err) {
				...
				console.log(
					`Deleted match for ${deleteMatchUser} successfully!`
				);
			...
			console.log(
				"Controller ERROR: Please provide at least currentUser!"
			);
			return res.status(400).json({
				message:
					"Controller ERROR: Please provide at least currentUser!",
		...
		console.log(err);
		return res.status(500).json({
			message: "Controller ERROR: Database failure when deleting match!",
		});
	}
}
...
export async function getRoom(req, res) {
	try {
		...
		if (currentUser) {
			...
			if (!matchExistUser1 && !matchExistUser2) {
				console.log(
					`Controller ERROR: currentUser (${currentUser}) is not in any match so there is no room id.`
				);
				return res.status(409).json(null);
			}

			if (matchExistUser1) {
				return res.status(200).json(matchExistUser1);
			} else {
				return res.status(200).json(matchExistUser2);
			}
		} else {
			console.log(
				"Controller ERROR: Please provide at least currentUser!"
		...
		console.log(err);
		return res.status(500).json({
			message: "Controller ERROR: Database failure when finding room id!",
		});
	}
}
